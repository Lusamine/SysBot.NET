...
{
    public sealed class EncounterBotRNGMonitorBS(PokeBotState cfg, PokeTradeHub<PB8> hub) : EncounterBotBS(cfg, hub)
    {
        ...
        protected override async Task EncounterLoop(SAV8BS sav, CancellationToken token)
        {
            ...
            if (Hub.Config.EncounterRNGBS.ResetRNGMonitorAdvances)
                TotalAdvances = 0;
            ...
            while (!token.IsCancellationRequested)
            {
                ...
                output = GetSeedMonitorOutput(_s0, _s1, Hub.Config.EncounterRNGBS.DisplaySeedMode);
